...
class RBT {
    RBTNode<T> *root;
    void leftRotate(RBTNode<T> *&point);
    void rightRotate(RBTNode<T> *&point);
    void fixTree(RBTNode<T> *& point);
...
//changed the doubleCR, doubleCCR, singleCR, singleCCR to just be two functions, rightRotate and leftRotate.

template <class T>
void RBT<T>::rightRotate(RBTNode<T> *&point) {
    RBTNode<T> *grandparent = point;
    RBTNode<T> *parent = point->left;
    // TODO: ADD ROTATION CODE HERE

    //grandparent's left pointer needs to go to parent's right
    grandparent->left = parent->right;
    if (parent->right != nullptr) {
        //if it isn't a nullptr, that node's parent needs to be grandpa
        parent->right->parent = grandparent;
    }
    //point parent's parent to grandparent's parent
    parent->parent = grandparent->parent;

    //if the grandparent's parent is a nullpointer, then root is the parent
...
template <class T>
void RBT<T>::leftRotate(RBTNode<T> *&point) {
    RBTNode<T> *grandparent = point;
...
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) {
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        fixTree(curr_node);
        //and just to make sure the root is always black
        root->color = BLACK;
...
template <class T>
void RBT<T>::fixTree(RBTNode<T> *& point) {
    //if the parent isn't red, this will not run
    while (getColor(point->parent) == RED) {
        //if the uncle of the point is Red, there is a color swap. If it is black, there must be a rotation. 
        //first figure out which side the parent is on
        if (point->parent == point->parent->parent->left) {
            RBTNode<T>* uncle = point->parent->parent->right;
            //now if it is a Red Node, then there is a color swap from the grandparent to its children
            if (getColor(uncle) == RED) {
                //swap colors of grandparent and its children
                ...
                swapColor(point->parent);
                swapColor(point->parent->parent);
                rightRotate(point->parent->parent);
            }
        }
        else {
            //just to further differentiate from the code above, since it is really similar, I called the sibling of the parent node the aunt in this one
            RBTNode<T>* aunt = point->parent->parent->left;
            if (getColor(aunt) == RED) {
                ...
                swapColor(point->parent);
                swapColor(point->parent->parent);
                leftRotate(point->parent->parent);
            }
        }
    }
}
